...
**Si 0 o 1 señales matchean** para todos los patrones → no ofrecer nada, pasar directo a P2.
...
### P4 · Sugiere arquitectura

Para **código**, consultar `references/architectures.md` y proponer según tamaño percibido:

- Mini (HTML+CSS+JS, demo, landing estática) → plana
- Chico (SPA con 1–2 features) → por tipo
- Mediano (multi-feature con estado compartido) → feature-based
- Grande (multi-equipo, dominios complejos) → Clean / Hexagonal
- Libre → el usuario describe la estructura

Si el usuario dice "no sé qué elegir", recomendar con justificación corta. Default amigable para principiante = por tipo o feature-based.

Para **creativo** e **híbrido**, la estructura está predefinida en `references/architectures.md`. Mostrarla y confirmar.

**Considerar principios activos del perfil**: leer `references/coding-principles.md` y la sección "Principios de código" de `profile.md`. Los principios activos influyen en la arquitectura sugerida:

- Si **Repository** está activo y hay DB → la arquitectura separa `domain/repositories/` (interfaces) de `infrastructure/persistence/` (implementaciones).
- Si **SOLID** está activo y el proyecto es mediano+ → separar interfaces de implementaciones en general (más carpetas, más archivos chicos).
- Si **YAGNI** está activo y el proyecto es chico → NO sobre-estructurar. Una landing estática no necesita `domain/` aunque Repository esté activo.
- Si **MVC/MVVM** está activo y el framework lo espera (Rails, Django, NestJS, Angular) → respetar su convención nativa.

Mencionar al usuario qué principios influyeron en la propuesta: *"Como tenés Repository y SOLID activos en tu perfil, separé `domain/` de `infrastructure/`. Si querés simplificar, podemos pasar a feature-based."*
...
**Aplicar principios de código al generar archivos**: cuando la skill genera código de scaffolding (componentes ejemplo, configs, archivos base), respetar los principios activos del perfil (ver `references/coding-principles.md`):

- Si "NO estilos inline" está activo y el stack es React → los componentes ejemplo usan clases (Tailwind, CSS Modules, styled-components según el stack), nunca `style={{}}`.
